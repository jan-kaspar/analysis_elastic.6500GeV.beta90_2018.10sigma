...
NewPad(false);
label("{\SetFontSizesXX elastic hits}");
...
draw(xyswitch, RootGetObject(f, dir+"/vertical/y_hist|y_hist"), "vl", black);
real y_beam = GetYResult();
draw((0, y_beam)--(6000, y_beam), blue+2pt);
limits((0, -20), (6000, +20), Crop);
...
draw(xyswitch, RootGetObject(f, dir+"/horizontal/horizontal graph fit/horizontal fit|ff"), "l", red+2pt);
limits((-3, -20), (+3, +20), Crop);
...
NewPad("$x\ung{mm}$", "$y\ung{mm}$");
draw(xyswitch, RootGetObject(f, dir+"/horizontal/horizontal graph fit/horizontal fit|ff"), "l", red+2pt);
draw((-10, y_beam)--(+10, y_beam), blue+2pt);
limits((-3, -20), (+3, +20), Crop);
